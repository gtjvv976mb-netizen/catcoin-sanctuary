...
export function addScenery({ statics, glass, glows, paint, place, sway, group, q }) {
  ...
  /* ── Dry-stone walls across the meadows (instanced stones and cap stones) ── */
  {
    const stones = [];
    for (const w of L.WALLS) {
      for (let i = 0; i < w.length - 1; i++) {
        ...
        for (let t = 0; t < len; t += 0.42) {
          const x = ax + ((bx - ax) * t) / len, z = az + ((bz - az) * t) / len;
          if (L.nearPath(x, z, 1.3) || L.inWater(x, z, 1.2) || L.inHall(x, z, 1)) continue;
          const y0 = H(x, z);
          for (let row = 0; row < 3; row++) {
            const s = rnd.range(0.22, 0.3) * (row === 2 ? 0.9 : 1);
            stones.push([x + rnd.range(-0.06, 0.06) + (row % 2) * 0.1, y0 + 0.13 + row * 0.25, z + rnd.range(-0.08, 0.08), ang + rnd.range(-0.3, 0.3), s, row === 2]);
          }
        }
      }
    }
    ...
    const m = new THREE.InstancedMesh(geo, new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: 0.9, flatShading: true }), stones.length);
