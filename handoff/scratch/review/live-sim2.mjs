...
let pre, sim; for (let t = 0; t < 6; t++) { pre = await rpc.getMultipleAccounts(addresses); sim = await rpc.call("simulateTransaction", [txBase64, { encoding: "base64", sigVerify: false, replaceRecentBlockhash: false, commitment: "processed", accounts: { encoding: "base64", addresses } }]); console.log("pre slot", pre.slot, "sim slot", sim.context.slot); if (pre.slot === sim.context.slot) break; await sleep(1500); }
