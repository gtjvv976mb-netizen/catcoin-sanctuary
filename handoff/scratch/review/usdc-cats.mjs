...
const found = new Map((process.env.MINTS ?? "").split(",").filter(Boolean).map((m) => [m, m.slice(0, 6)]));
const progs = new Map();
{ const r = await rpc.getMultipleAccounts([...found.keys()]); [...found.keys()].forEach((m, i) => progs.set(m, r.accounts[i]?.owner ?? null)); }
...
async function run(M, J, hop, tok, side, amountRaw) {
  ...
  try {
    ...
    const c = M.checkSwapTransaction({ txBase64: built.swapTransaction, wallet, inputMint, outputMint, inputProgram: side === "buy" ? T : progs.get(tok), outputProgram: side === "buy" ? progs.get(tok) : T, amountRaw, quote: raw, slippageCapBps: 100, lookupTables: tables, maxPriorityFeeLamports: 50_000, allowedPairs: new Set([`${USDC}>${tok}`, `${tok}>${USDC}`]), ...x });
