...
section("REVIEW: custody address pre-funded by an outsider (0-byte rent minimum read live 2026-09-25: 650,240 lamports)");
for (const [label, token, universe] of [["hop through SOL (KITTY)", KITTY, [POPCAT, KITTY]], ["direct (POPCAT)", POPCAT, [POPCAT, KITTY]]]) {
  const { w, r, AUTO, held } = await liveRig({ spec: { universe } });
  const ata = w.chain.ataOf(AUTO, token);
  w.chain.st.lamports.set(ata, 650_240n);               // anyone can send this; no signature of the wallet needed
  ...
  console.log(`  ${label}: fill=${fill ? "yes" : "no"} refusal=${refusal?.clause} — ${refusal?.message?.slice(0, 230)}`);
  console.log(`    sent=${w.chain.st.sent.size} walletLamportsMoved=${lamBefore - w.chain.st.lamports.get(AUTO)} usdcMoved=${usdcBefore - held(USDC)}`);
  // and again on the next tick: the address never gets created, so every buy of it is refused
  w.decisions.push({ rationale: "buy again", actions: [buy(token, 10, "trend")] });
  await r.runNow(); await r.tick();
  console.log(`    second attempt: ${journalOf(r, "refusal")[0]?.clause}; sent=${w.chain.st.sent.size}`);
}
