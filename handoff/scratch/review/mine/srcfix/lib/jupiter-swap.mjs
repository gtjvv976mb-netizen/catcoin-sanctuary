 ...
 *     20 KITTY had no route. So the agent asks at 24, and the SHAPE is held by the check, not
...
export function createJupiterClient({
  ...
  return Object.freeze({
    /** An exact-in quote. By default on DIRECT routes only (the xStock venue): the pool
     *  pairs the two mints, and a hop through a third token would put accounts on the
     *  wallet this venue never checks. With `solHop` (the agent) it asks for a short route
     *  instead — onlyDirectRoutes=false, restrictIntermediateTokens=true, maxAccounts=24 —
     *  which on the recorded morning came back direct or one hop through SOL for every
     *  preset cat coin. Jupiter cannot be told WHICH intermediate; checkQuote holds that. */
    quote({ inputMint, outputMint, amountRaw, slippageBps, priority = "entry", solHop = false }) {
      return request("/quote", { priority, query: {
        ...
        ...(solHop ? { onlyDirectRoutes: "false", restrictIntermediateTokens: "true", maxAccounts: String(JUPITER_HOP_MAX_ACCOUNTS) }
 ...
 *  created (their lamports after the simulation), and not one lamport more. More is the
 *  wallet's SOL spent as trade input — the engine's own guard allows up to its fee and rent
 *  caps; this holds the swap to what it itself explains: `sol_as_input`. (A node whose
 *  simulation does not charge the fee to the payer shows less spent, never more: the bound
 *  is then looser by the fee alone.) */
export function checkNativeSpend({ spentLamports, signatures = 1, priorityFeeLamports, rentLamports = 0n }) {
  ...
  if (BigInt(spentLamports) > allowed)
    throw new SwapCheckError("sol_as_input", `the simulation takes ${spentLamports} lamports of the wallet's SOL; the fee (${fee}) and the rent of the accounts it creates (${rentLamports}) explain ${allowed}: the wallet's SOL pays fees and rent, never the trade`);
}
