...
const TOK = { TROLL: "BdGuUUxfp74PnyEYAFnEBFrsHVdDh5PTRHgqqbHKmoon", VIBE: "DFVeSFxNohR5CVuReaXSz6rGuJ62LsKhxFpWsDbbjups", MOON: "241aTYhVXZ4WBVSFpfY37RqoCGBQ73KiRFAKvTtnmoon", SOLCATS: "HAgX1HSfok8DohiNCS54FnC2UJkDSrRVnT38W3iWFwc8", SNEKO: "8x382FRw85zkkA8ZDvky8HkKWPpJugi2ujoZeyHmpump", CATEAPE: "6y6j2SQbUtmAF8uMkk1Zf7ERgrq81zGAmZJjwkQV3RrQ", MEW: "MEW1gQWJ3nEXg2qgERiKu7FAFj79PHvQVREQUzScPP5", POPCAT: "7GCihgDB8fe6KNjn2MYtkzZcRjQy3t9GHdC8uHYmW2hr", KITTY: "4N4DnNo3qpPks9aQCkcWkzoir8tnvT6diS4TnnZibonk", GRUMPY: "GRUmPYbiTpq9ZPy5LAqBMMze7kErf5dEX2i9qYfwoSmR" };
...
const out = [];
const PROGS = new Map(); { const ms = Object.values(TOK); const r = await rpc.getMultipleAccounts(ms); ms.forEach((m, i) => PROGS.set(m, r.accounts[i]?.owner)); }
for (const c of cases) {
  ...
  try {
    ...
    const checked = checkSwapTransaction({ txBase64, wallet, inputMint, outputMint, inputProgram: side === "buy" ? T : PROGS.get(t), outputProgram: side === "buy" ? PROGS.get(t) : T, amountRaw, quote: raw, slippageCapBps: 100, lookupTables: tables, maxPriorityFeeLamports: 50_000, allowedPairs: pairs, solHop: true });
    ...
    const tokenAta = associatedTokenAddress(wallet, t, PROGS.get(t)), settlementAta = associatedTokenAddress(wallet, s, T);
