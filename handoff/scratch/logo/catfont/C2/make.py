...
BN = dict(z=1.15, y0=180,                 # scene: scaled to width 1500*z, crop (0, y0, 1500, y0+500)
          ...
          tag_gap=30, house_x=1085, tag_dy=dict(a=4, b=2))                     # tagline block: this far right of "Catcoin"
BANNER_PICK = 'b'
...
def paw_icon(size, fill, outline, ow):
    """Paw print (pad + 4 toe beans), drawn at 4x and downsampled."""
    k = 4
    S = size * k
    L = Image.new('RGBA', (S, S), (0, 0, 0, 0))
    d = ImageDraw.Draw(L)
    shapes = [('e', (0.22, 0.46, 0.78, 0.94)),
              ('e', (0.04, 0.30, 0.26, 0.56)), ('e', (0.22, 0.06, 0.44, 0.36)),
              ('e', (0.56, 0.06, 0.78, 0.36)), ('e', (0.74, 0.30, 0.96, 0.56))]
    for grow, col in ((ow * k, outline), (0, fill)):
        for _, (a, b, c, e) in shapes:
            d.ellipse((a * S - grow, b * S - grow, c * S + grow, e * S + grow), fill=col)
    return L.resize((size, size), LANCZOS)


def tagline_block(variant):
    """Tagline + url as a designed element in the logo's language (Gluten, cream, thick plum outline)."""
    t1, t2 = 'Where all catcoins live', 'catcoinsanctuary.com  \u00b7  $CATSANC'
    PINK = (255, 150, 178)
    k = 3                                                   # supersample
    B = Image.new('RGBA', (1000 * k, 300 * k), (0, 0, 0, 0))
    d = ImageDraw.Draw(B)
    if variant == 'a':
        # plum pill with cream Gluten tagline + paw; url in amber Figtree underneath on a slimmer pill
        g = ImageFont.truetype(GLUTEN, 40 * k)
        g.set_variation_by_axes([800])
        f2 = figtree(21 * k, 700)
        b1 = d.textbbox((0, 0), t1, font=g)
        paw = 34 * k
        pw = (b1[2] - b1[0]) + paw + 3 * 18 * k
        ph = 64 * k
        x0, y0 = 20 * k, 20 * k
        d.rounded_rectangle((x0, y0, x0 + pw, y0 + ph), ph // 2, fill=PLUM + (235,), outline=AMBER, width=3 * k)
        px = x0 + 20 * k
        B.alpha_composite(paw_icon(paw, PINK, PLUM, 0), (px, y0 + (ph - paw) // 2))
        tx = px + paw + 14 * k
        ty = y0 + ph // 2 - (b1[1] + b1[3]) // 2
        d.text((tx, ty), t1, font=g, fill=CREAM)
        b2 = d.textbbox((0, 0), t2, font=f2)
        w2, h2 = b2[2] - b2[0] + 36 * k, 38 * k
        ux = x0 + (pw - w2) // 2
        uy = y0 + ph + 10 * k
        d.rounded_rectangle((ux, uy, ux + w2, uy + h2), h2 // 2, fill=(46, 16, 38, 190))
        d.text((ux + 18 * k - b2[0], uy + h2 // 2 - (b2[1] + b2[3]) // 2), t2, font=f2, fill=AMBER)
    else:
        # sticker style: cream Gluten with the thick plum outline like the wordmark, paw beans, url tag below
        g = ImageFont.truetype(GLUTEN, 50 * k)
        g.set_variation_by_axes([800])
        f2 = figtree(22 * k, 750)
        b1 = d.textbbox((0, 0), t1, font=g, stroke_width=7 * k)
        x0, y0 = 20 * k, 20 * k
        paw = 40 * k
        d.text((x0 + paw + 12 * k - b1[0], y0 - b1[1]), t1, font=g, fill=CREAM, stroke_width=7 * k, stroke_fill=PLUM)
        th = b1[3] - b1[1]
        B.alpha_composite(paw_icon(paw, PINK, PLUM, 5), (x0, y0 + (th - paw) // 2))
        tw = paw + 12 * k + b1[2] - b1[0]
        b2 = d.textbbox((0, 0), t2, font=f2)
        w2, h2 = b2[2] - b2[0] + 40 * k, 40 * k
        ux, uy = x0 + (tw - w2) // 2, y0 + th + 8 * k
        d.rounded_rectangle((ux, uy, ux + w2, uy + h2), h2 // 2, fill=AMBER, outline=PLUM, width=4 * k)
        d.text((ux + 20 * k - b2[0], uy + h2 // 2 - (b2[1] + b2[3]) // 2), t2, font=f2, fill=PLUM)
        B = B.rotate(3, resample=Image.Resampling.BICUBIC, expand=True)
    return B.resize((B.width // k, B.height // k), LANCZOS)


def make_banner(variant='a'):
    W, H = 1500, 500
    ...
    T = tagline_block(variant)
    # centre the block in the sky between "Catcoin" and the cottage, vertically on "Catcoin"'s letters
    tb = bbox(np.asarray(T.getchannel('A')) > 20)
    T = T.crop(tb)
    # big tagline centred under "Sanctuary", in the open meadow
    room = BN['house_x'] - c_box[2] - 2 * BN['tag_gap'] + 20
    if T.width > room:                                   # fit between "Catcoin" and the cottage
        T = scaled_rgba(T, room / T.width)
    tb = (0, 0, T.width, T.height)
    cx = (c_box[2] + BN['house_x']) / 2
    cy = (c_box[1] + c_box[3]) / 2 + BN['tag_dy'][variant]
    L = Image.new('RGBA', (W, H), (0, 0, 0, 0))
    L.alpha_composite(T, (int(cx - (tb[0] + tb[2]) / 2), int(cy - (tb[1] + tb[3]) / 2)))
    T = L
    im.alpha_composite(soft_shadow(T, 5, 8, 0.55))
...
if __name__ == '__main__':
    ...
    for v in ('a', 'b'):
        bv, vi = make_banner(v)
        for box in (vi['cat'], vi['san'], vi['tag']):
            assert box[1] >= 40 and box[3] <= 420, (v, box)
            assert not (box[0] < 420 and box[3] > 330), (v, box)
        assert not vi['corner'] and vi['d_av'] >= X_AV_R, vi
        bv.save(os.path.join(HERE, f'banner-{v}.png'))
        print(f"banner-{v}: tagline block {vi['tag']}")
    bn, bi = make_banner(BANNER_PICK)
