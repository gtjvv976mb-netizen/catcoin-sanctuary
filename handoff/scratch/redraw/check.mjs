...
for (const t of T) {
  ...
  const r = {
    ...
    endsWithDisclosure: e.description.endsWith(disc),
    ...
    checkFields_disclosureOnly: rules.checkFields({ description: disc }),
  };
...
fs.writeFileSync("check.out.json", JSON.stringify(out,null,1));
