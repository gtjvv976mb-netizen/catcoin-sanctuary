 ...
 *
 * THE RENT OF AN ACCOUNT THIS TRANSACTION OPENED is what it gained. That is all of it for an
 * address that held nothing; for one that held lamports first — anyone can send SOL to the
 * wallet's future coin-account address, and the associated-token program then tops it up
 * rather than paying in full (1,513,840 lamports either way on the recorded LMEOW account,
 * fixtures/agent/mainnet-rent-and-slot-reads.json) — it is only the top-up. Such an account is
 * known two ways: it becomes a token account of the wallet here (in postTokenBalances under
 * the wallet, absent from preTokenBalances), or the caller names it in `opened` — the
 * addresses its own simulation saw this transaction open (an AMM's account for the wallet,
 * say), matched to the balance arrays through the transaction's account keys. Without that,
 * a buy into a pre-funded address would land and then be refused here as a drain.
 */
export function fillFromTransaction(tx, { wallet, mint, side, quoteMint = null, quoteDecimals = null, opened = null }) {
  const meta = tx?.meta;
  ...
  const openedHere = openedIndexes(tx, { wallet, opened });
  let rent = 0n;
  for (let i = 1; i < pre.length; i++) {
    const before = BigInt(pre[i]), after = BigInt(post[i]);
    if (before === 0n && after > 0n) rent += after;
    else if (openedHere.has(i) && after > before) rent += after - before;
  }
...
/** The balance-array indexes of the accounts this transaction opened with lamports already in
 *  them: the wallet's new token accounts, read off the token balances, and the addresses in
 *  `opened`, placed through the transaction's own account keys (the static keys, then the
 *  lookup tables' writable and read-only addresses, the order the balance arrays follow). */
function openedIndexes(tx, { wallet, opened }) {
  const meta = tx.meta;
  const out = new Set();
  const had = new Set((meta.preTokenBalances ?? []).map((b) => b?.accountIndex));
  for (const b of meta.postTokenBalances ?? []) if (b?.owner === wallet && Number.isInteger(b.accountIndex) && !had.has(b.accountIndex)) out.add(b.accountIndex);
  if (Array.isArray(opened) && opened.length) {
    const keys = [...(tx.transaction?.message?.accountKeys ?? []), ...(meta.loadedAddresses?.writable ?? []), ...(meta.loadedAddresses?.readonly ?? [])]
      .map((k) => (typeof k === "string" ? k : k?.pubkey ?? null));
    const want = new Set(opened.map(String));
    keys.forEach((k, i) => { if (want.has(String(k))) out.add(i); });
  }
  return out;
}
