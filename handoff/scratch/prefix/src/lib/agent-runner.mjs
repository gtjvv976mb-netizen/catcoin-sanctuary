 ...
 *      untouched, the engine's simulate guard (its read and its simulation on one slot), the
 *      exact input, no wrapped SOL left in the wallet, its SOL moved by the fee and the rent of
 *      what the swap opens alone, and both accounts safe after the simulation; only then the
 *      engine's signSendConfirm, fixed to the AUTOPILOT WALLET
...
import {
  ...
  checkRouteMints, checkIntermediateLeft, checkNativeSpend, checkOpenedCustody, rentFromSysvar, rentOfOpenedAccounts, SYSVAR_RENT, JupiterError, SwapCheckError,
...
export function createAgentRunner({
   ...
   * USDT at all, so every quote here is asked with `solHop` (jupiter-swap.mjs): Jupiter is
   * asked for a direct route first, exactly as before, and only when it has none for a route
   * that may pass through exactly one intermediate, SOL — never two, never another token,
   * never split between ways — and the 2% buy-impact cap is the WHOLE route's. Paper and live
   * take the same quote. A live hop is built on Jupiter's shared accounts, so the SOL in the
   * middle sits in Jupiter's own account and never in the autopilot wallet; the check before
   * signing proves that from the bytes (checkSwapTransaction), from the chain (every token
   * account the route writes holds the settlement token, SOL or the listed token, before the
   * simulation and after it: checkRouteMints) and from the simulation (the wallet's
   * wrapped-SOL account is untouched: checkIntermediateLeft).
   *
   * THE WALLET'S SOL, ON EVERY SWAP, hop or direct, may move by the transaction's own fee and
   * the rent of the accounts it opens and nothing more (checkNativeSpend): the autopilot's
   * SOL pays fees and is never trade input. So the simulation reads EVERY account the swap
   * writes, before and after, on one slot (the engine's guard with `pinSlot`: the wallet is
   * shared with the sniper lane and CashCat, whose transactions can land between two reads),
   * and the rent is what each account the swap opens gained, up to its rent-exempt minimum at
   * the chain's own rate (rentOfOpenedAccounts, the rent sysvar read with the swap's accounts):
   * the wallet's new coin account, an AMM's own account for the wallet — PumpSwap opens one on
   * a wallet's first buy — and an address someone sent SOL to first, charged the top-up only.
   * No account the swap opens may be another token account of the wallet (checkOpenedCustody).
   */
  async function swap({ side, entry, amountRaw, priority }) {
    ...
    /* The accounts the swap writes, and the chain's rent rate, in one read. */
    const n = checked.writableAddresses.length;
    const writable = await rpc.getMultipleAccounts([...checked.writableAddresses, SYSVAR_RENT]);
    const writableBefore = (writable.accounts ?? []).slice(0, n);
    const rent = rentFromSysvar(writable.accounts?.[n] ?? null);
    checkWritableCustody({ wallet, writableAddresses: checked.writableAddresses, accounts: writableBefore, allowed: [settlementAta, tokenAta] });
    const hopMints = checked.intermediate ? [s.mint, checked.intermediate, entry.mint] : null;
    if (hopMints) checkRouteMints({ writableAddresses: checked.writableAddresses, accounts: writableBefore, mints: hopMints });
    /* The simulation reads [wallet, token, settlement, the wallet's wrapped-SOL account, then
       every other account the swap writes], the read before and the simulation on one slot. */
    const own = [wallet, tokenAta, settlementAta, checked.walletIntermediateAta];
    const watch = [checked.walletIntermediateAta, ...checked.writableAddresses.filter((a) => !own.includes(a))];
    const simulated = [wallet, tokenAta, settlementAta, ...watch];
    const guard = await f.simulateGuard({
      ...
      quote: { mint: s.mint, ata: settlementAta, decimals: s.decimals, symbol: s.symbol }, watch, pinSlot: true,
    });
    ...
    if (hopMints) checkRouteMints({ writableAddresses: simulated, accounts: guard.post ?? [], mints: hopMints });
    checkOpenedCustody({ wallet, addresses: simulated, before: guard.pre, after: guard.post, allowed: [tokenAta, settlementAta] });
    /* The rent the wallet may pay: what each account this swap opens gained, up to its minimum. */
    const rentPaid = rentOfOpenedAccounts({ addresses: simulated, before: guard.pre, after: guard.post, rent });
    checkNativeSpend({ spentLamports: guard.spend, signatures: checked.signatures, priorityFeeLamports: checked.priorityFeeLamports, rentLamports: rentPaid.lamports });
    ...
    let fill;
    try { fill = fillFromTransaction(signed.tx, { wallet, mint: entry.mint, side, quoteMint: s.mint, quoteDecimals: s.decimals, opened: rentPaid.opened.map((o) => o.address) }); }
