...
/** How many read-and-simulate pairs a `pinSlot` guard takes before it refuses: at the seven
 *  in twenty that missed on the public RPC on 2026-09-25, six misses running is under one
 *  swap in five hundred — and a refused sell is simply tried again on the next tick. */
...
export function createHawkEngine({
   ...
   *
   *  `pinSlot` (the agent's): the read before and the simulation are two calls, and the
   *  wallet is shared — the sniper lane and CashCat sign with the same autopilot key — so a
   *  transaction of theirs can land between them, and every "before → after" here would then
   *  span two states: SOL leaving in the gap reads as the swap's spend (an honest swap
   *  refused), SOL arriving hides the same amount of it. The other lanes judge against caps
   *  wide enough to absorb that; the agent holds its swap to the fee and rent to the lamport
   *  (jupiter-swap.mjs checkNativeSpend), so with `pinSlot` the pair is taken again until
   *  both answer from ONE slot, and only that pair is judged. Seven of twenty back-to-back
   *  pairs on the public RPC answered one slot apart on 2026-09-25, and every same-slot pair
   *  agreed on the wallet to the lamport (fixtures/agent/mainnet-rent-and-slot-reads.json);
   *  SIMULATION_PIN_TRIES pairs that all miss are refused, `simulation_unpinned`. */
  async function simulateGuard({ txBase64, wallet, ata, mint, side, expected, quote = null, watch = [], pinSlot = false }) {
    const addresses = [...(quote ? [wallet, ata, quote.ata] : [wallet, ata]), ...watch];
    let pre, sim;
    for (let tries = 1; ; tries++) {
      pre = await rpc.getMultipleAccounts(addresses);
      sim = await rpc.simulateTransaction(txBase64, { addresses });
      if (!pinSlot || sim?.err) break;
      const readSlot = Number(pre?.slot), simSlot = Number(sim?.contextSlot);
      if (Number.isSafeInteger(readSlot) && readSlot > 0 && readSlot === simSlot) break;
      if (tries >= SIMULATION_PIN_TRIES)
        throw new TxError("simulation_unpinned", `the read before the simulation and the simulation answered from different slots ${tries} times running (last ${pre?.slot ?? "none"} and ${sim?.contextSlot ?? "none"}): what the swap spends cannot be measured on one state — nothing was signed`);
    }
