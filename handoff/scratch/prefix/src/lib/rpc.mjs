...
export function createRpc({ url, fetchImpl = globalThis.fetch, timeoutMs = 12_000 } = {}) {
  ...
  return Object.freeze({
    ...
    /** The node's own simulation of the unsigned bytes, with post-state for `addresses`, and
     *  `contextSlot`: the slot it simulated on, as getMultipleAccounts returns `slot`, so a
     *  caller can tell whether a read and a simulation saw the same state (engine.mjs
     *  simulateGuard's `pinSlot`). */
    async simulateTransaction(txBase64, { addresses = [], commitment = "processed" } = {}) {
      ...
      return value && typeof value === "object" ? { ...value, contextSlot: Number(r?.context?.slot) || null } : value;
    },
