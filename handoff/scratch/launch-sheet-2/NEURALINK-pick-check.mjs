// Checker pass for NEURALINK (prestock). Re-runs every rule on the three drafts.
...
const drafts = JSON.parse(fs.readFileSync(`${DIR}/NEURALINK-coins.json`, "utf8"));
const LOOKS = {
  SILKSTRIPE: "Sleek silver tabby with very fine charcoal pinstripes and a white chin, green eyes, slim build, wears nothing.",
  HUSHPAW: "Smooth solid blue-grey shorthair with pale gold eyes, wears nothing.",
  LILACNAP: "Cream cat with lilac pinkish-grey points on ears, face and paws, pale blue eyes, wears nothing.",
};
const coins = drafts.map((c) => ({ id: c.ticker, ...c, disc: DISC, look: LOOKS[c.ticker] }));

const own = ["NEURALINK", "Neuralink", "Neuralink Corp", "Neuralink PreStocks", "Neural", "NEURA", "NEUR", "NRLK", "Link", "The Link", "N1", "R1", "Telepathy", "Blindsight",
  "PRIME", "CONVOY", "CAN-PRIME", "PreStocks", "PreStock", "prestock", "pre-IPO", "SPV", "BCI", "implant", "brain chip", "neuro", "neuron",
  "Elon", "Musk", "Elon Musk", "Maye", "Schrodinger", "Schrödinger", "Erwin", "Max Hodak", "Hodak", "Benjamin Rapoport", "Rapoport", "Dongjin Seo", "DJ Seo", "Seo", "Dongjin",
  "Paul Merolla", "Merolla", "Philip Sabes", "Sabes", "Tim Gardner", "Tim Hanson", "Hanson", "Vanessa Tolosa", "Tolosa", "Jared Birchall", "Birchall",
  "Gatsby", "Marvin", "Gertrude", "Pager", "Code", "Pong", "MindPong", "Blue Belle", "Umbreon", "Noland Arbaugh", "Arbaugh", "Noland",
  "xAI", "Grok", "Tesla", "SpaceX", "Starlink", "Optimus", "Boring Company", "X Corp"];

const notes = notesFor(S.STOCK_PAIRS);
const xstockTerms = S.STOCK_PAIRS.flatMap((p) => [...S.pairTerms(p, notes), ...(ROWS[p.symbol]?.extra ?? [])]);
const otherTerms = [];
for (const f of fs.readdirSync(DIR).filter((f) => /^[A-Z0-9]+-(pick-)?check\d?\.mjs$/.test(f) && !f.startsWith("NEURALINK"))) {
  const src = fs.readFileSync(`${DIR}/${f}`, "utf8");
  const m = src.match(/stock(?:Terms)?\s*[:=]\s*(?:\{\s*stock:\s*)?(\[[^\]]*\])/);
  if (!m) continue;
  try { otherTerms.push(...Function(`return ${m[1]}`)()); } catch {}
}
const ownN = new Set(own.map((o) => normalize(o)));
const others = [...new Set([...xstockTerms, ...otherTerms])].filter((t) => !ownN.has(normalize(t)));

const earlier = [];
try { for (const c of JSON.parse(fs.readFileSync(`${SP}/launch-sheet/EDITOR/final.json`, "utf8")).final) earlier.push({ name: c.name, symbol: c.ticker, from: "launch-sheet final" }); } catch (e) { earlier.push({ err: String(e) }); }
for (const f of fs.readdirSync(DIR).filter((f) => /-pick\.json$/.test(f) && !f.startsWith("NEURALINK"))) {
...
for (const f of fs.readdirSync(DIR).filter((f) => /-coins\d?\.json$/.test(f) && !f.startsWith("NEURALINK"))) {
...
const MEME = new Set(["POPCAT", "MEW", "MICHI", "GRUMPY", "KITTY", "CAT", "CATS", "NYAN", "KITTEN", "TABBY", "MOCHI", "MOG", "PUSS", "MEOW", "PURR", "GINGER", "NEKO", "WIF", "CATWIF", "CWIF", "MANEKI", "SC", "LION", "LEO", "ROAR", "MANE", "FATCAT", "SCHRODI", "SCHRO", "SILK", "HUSH", "LILAC"]);
...
for (const c of coins) {
  ...
  const o = { id: c.id, name: c.name, ticker: c.ticker,
    ...
    stockTickerOrRootInTicker: ["NEURALINK", "NEURAL", "NEURA", "NEUR", "LINK", "NRLK"].filter((t) => c.ticker.includes(t)),
    disclosureEndsRight: description.endsWith(`A cat coin priced in ${PAIR}. Not affiliated with Neuralink, PreStocks or StonkFun. No intrinsic value; not financial advice.`),
  };
  ...
  o.earlierCollision = earlier.filter((e) => e.name && (normalize(e.name) === normalize(c.name) || alnum(e.symbol) === alnum(c.ticker) || normalize(e.name).split(" ")[0] === normalize(c.name).split(" ")[0]));
...
const MINT = "PrekqLJvJ3qVdXmBGDiexvwUTF4rLFDa6HWS4HJbw9S";
let mint = {};
try {
  ...
  const ei = await rpc("getEpochInfo", []);
  mint.epoch = ei?.epoch; mint.slotsLeftInEpoch = ei ? ei.slotsInEpoch - ei.slotIndex : null;
  mint.hoursLeftApprox400ms = mint.slotsLeftInEpoch ? +(mint.slotsLeftInEpoch * 0.4 / 3600).toFixed(1) : null;
