...
const stockTerms = { stock: ["GPRO", "GPR", "GoPro", "Go Pro", "GoPro Inc", "Hero", "Karma", "Quik", "Fusion", "Session", "Chesty", "Volta", "Enduro", "Be a Hero",
  ...
  "Backpack", "Backpack Securities", "Sunrise", "Wormhole", "Insta360", "Oscar", "skateboard", "skateboarding", "Laser Cats", "lion", "lions", "camera", "action cam"] };
const banned = new Set(["POPCAT","MEW","MICHI","GRUMPY","KITTY","CAT","CATS","NYAN","KITTEN","TABBY","MOG","MANEKI","HOBBES","WIF","BONK","CATWIF","PURR","MEOW","SIMON","GIKO","BOOK","NUB","SNOWBALL","OSCAR","GPRO","GOPRO"]);
const results = [];
for (const c of coins) {
  const desc = `${c.blurb} ${DISC}`;
  const out = { name: c.name, ticker: c.ticker, nameLen: c.name.length, blurbLen: c.blurb.length, descLen: desc.length, tickerFmt: R.TICKER.test(c.ticker), tickerHasRoot: /GPR|GOPRO|^GO|PRO/.test(c.ticker) };
...
console.log(JSON.stringify({ discLen: DISC.length, disclosureOnly: R.checkFields({ description: DISC }), results }, null, 1));
