...
const stockTerms = { stock: ["MU", "MUx", "Micron", "Micron Technology", "Crucial", "Backpack", "Backpack Securities", "Sunrise", "Wormhole", "Sanjay", "Mehrotra", "Sanjay Mehrotra", "Lexar", "Boise"] };
const banned = new Set(["POPCAT","MEW","MICHI","GRUMPY","KITTY","CAT","CATS","NYAN","KITTEN","TABBY","MU","MUX","MICRON","KEYCAT","SIGMA","MOG","MANEKI","HOBBES","GME","WIF","BONK","SC","CATWIF","MANEKI","PURR","MEOW"]);
const results = [];
for (const c of coins) {
  const desc = `${c.blurb} ${DISC}`;
  const out = { name: c.name, ticker: c.ticker, nameLen: c.name.length, blurbLen: c.blurb.length, descLen: desc.length, tickerStartsMU: c.ticker.startsWith("MU") };
