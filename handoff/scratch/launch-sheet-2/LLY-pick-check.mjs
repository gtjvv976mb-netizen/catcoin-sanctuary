...
const coins = JSON.parse(fs.readFileSync(`${DIR}/launch-sheet-2/LLY-coins.json`, "utf8"));
const stockTerms = { stock: ["LLY", "LLYx", "Lilly", "Eli Lilly", "Eli", "Lily", "Lilies", "Eli Lilly and Company", "Backpack", "Backpack Securities", "Sunrise", "Wormhole",
  ...
  "red script", "Sage Therapeutics", "Supernus", "Pipette"] };
...
// founder history (for the apothecary look basis)
let wiki = null;
try {
  const r = await fetch("https://en.wikipedia.org/w/api.php?action=query&prop=extracts&explaintext=1&format=json&titles=Eli_Lilly_(pharmacist)", { headers: { "user-agent": "cat-sanctuary-check/1.0" } });
  const j = await r.json(); const t = Object.values(j.query.pages)[0].extract ?? "";
  wiki = { status: r.status, len: t.length, hits: t.split(/(?<=\.)\s+/).filter((s) => /drug ?store|apothecar|pharmac(y|ist)|Greencastle|apprentic/i.test(s)).slice(0, 8) };
} catch (e) { wiki = { error: String(e) }; }
