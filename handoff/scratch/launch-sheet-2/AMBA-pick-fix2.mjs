...
const c = { name: "Vignette the Seal-Point Cat", ticker: "VIGNETTE",
  blurb: "Vignette, a cream seal-point cat with a dark brown mask, ears, paws and tail, gazes from the porch step with wide sky-blue eyes and a slow blink.",
  look: "A normal four-legged cream cat with seal-point colouring: a dark brown mask, ears, paws and tail, and wide sky-blue eyes. It is based on camera image processing, where a vignette is the darker edge around a bright picture, like dark points around a pale coat. It wears nothing." };
...
o.jup = {};
for (const q of [c.ticker, "VIGNEPAW", "Vignette"]) {
  const r = await fetch(`https://lite-api.jup.ag/tokens/v2/search?query=${encodeURIComponent(q)}`);
  const j = await r.json(); const arr = Array.isArray(j) ? j : (j.tokens ?? []);
  o.jup[q] = { status: r.status, results: arr.length, top: arr.slice(0, 5).map(t => `${t.symbol}|${t.name}|verified=${t.isVerified}`), symbolMatches: arr.filter(t => String(t.symbol).toUpperCase() === q.toUpperCase()).map(t => ({ name: t.name, isVerified: t.isVerified, id: t.id })) };
