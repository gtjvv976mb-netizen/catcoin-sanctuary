// Checker pass for SNAP (backpack). Re-runs every rule on the three drafts.
...
const drafts = JSON.parse(fs.readFileSync(`${DIR}/SNAP-coins.json`, "utf8"));
const LOOKS = [
  "Lean tawny short-haired house cat, cream chest and muzzle, dark-tipped tail, gold-brown eyes, wears nothing.",
  "Bright orange mackerel tabby house cat with bold dark stripes, white chin, gold eyes, wears nothing.",
  "Sleek black-and-white tuxedo house cat with a white face blaze, green eyes, wears nothing.",
];
const coins = drafts.map((d, i) => ({ id: `D${i + 1}`, name: d.name, ticker: d.ticker, blurb: d.blurb, disc: DISC, look: LOOKS[i] }));
...
for (const c of coins) {
  ...
  const o = { id: c.id, name: c.name, ticker: c.ticker,
    ...
    notStockTicker: !["SNAP", "SNP"].some((t) => c.ticker.startsWith(t) || c.ticker.endsWith(t)),
...
const MINT = "SNAPcESrvnH8yUdgeMF6xm1hym9b6hW6s8YeqeHdZFz";
let mint = {};
try {
