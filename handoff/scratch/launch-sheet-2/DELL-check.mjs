...
const stockTerms = { stock: ["DELL", "Dell", "Dell Technologies", "Dell Technologies Inc", "Dell Inc", "PCs Limited", "Michael Dell", "Michael", "Susan Dell", "Jeff Clarke", "Round Rock",
  ...
  "Dude", "Dell Dude", "Ben Curtis", "Steven", "Reallusion", "Live Cam Avatar", "Cam Avatar", "Webcam Central", "Webcam", "Rod Ponton", "Ponton", "lawyer", "not a cat", "Zoom", "Skype", "filter",
  "Backpack", "Backpack Securities", "Sunrise", "Wormhole"] };
const banned = new Set(["POPCAT","MEW","MICHI","GRUMPY","KITTY","CAT","CATS","NYAN","KITTEN","TABBY","MOG","MANEKI","HOBBES","WIF","BONK","CATWIF","PURR","MEOW","SIMON","GIKO","BOOK","NUB","SNOWBALL","KEYBOARD","DELL","DELLX"]);
const results = [];
for (const c of coins) {
  const desc = `${c.blurb} ${DISC}`;
  const out = { name: c.name, ticker: c.ticker, nameLen: c.name.length, blurbLen: c.blurb.length, descLen: desc.length, tickerHasDellRoot: c.ticker.includes("DELL") || c.ticker.includes("DEL") };
