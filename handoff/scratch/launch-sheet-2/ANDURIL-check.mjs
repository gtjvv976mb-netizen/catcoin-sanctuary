...
const stockTerms = { stock: ["ANDURIL", "Anduril", "Anduril Industries", "ANDU", "ANDUR", "PreStocks", "PreStock", "Prestocks", "Palmer", "Luckey", "Palmer Luckey", "Schimpf", "Brian Schimpf", "Trae", "Trae Stephens",
  "Lattice", "Sentry", "Ghost", "Anvil", "Roadrunner", "Barracuda", "Fury", "Copperhead", "Pulsar", "Omen", "Menace", "Wisp", "EagleEye", "Eagle Eye", "Thunder", "Spyglass", "Spark", "Arsenal", "Dive",
  "Lynx", "Team Lynx", "Rheinmetall", "XM30", "Mears", "Tolkien", "Narsil", "Flame of the West", "sword", "Zuckerberg", "Meta", "cat ears", "helmet", "Hello Kitty", "Red Cat", "warfighter", "military", "defense", "drone"] };
const banned = new Set(["POPCAT","MEW","MICHI","GRUMPY","KITTY","CAT","CATS","NYAN","KITTEN","TABBY","MOG","MANEKI","HOBBES","WIF","BONK","CATWIF","PURR","MEOW","SIMON","GIKO","BOOK","NUB","SNOWBALL","LYNX","ANDURIL"]);
const results = [];
for (const c of coins) {
  const desc = `${c.blurb} ${DISC}`;
  const out = { name: c.name, ticker: c.ticker, nameLen: c.name.length, blurbLen: c.blurb.length, descLen: desc.length, tickerHasAndRoot: /^AND/.test(c.ticker) || c.ticker.includes("ANDURIL") };
