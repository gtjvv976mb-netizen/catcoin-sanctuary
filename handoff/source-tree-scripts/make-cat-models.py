...
def drop_small(js, binc, frac):
    """Remove disconnected pieces (vertices welded by position) with fewer than frac of a primitive's
    triangles, e.g. a stray fragment floating above the head. Appends new index data to the binary chunk."""
    out = bytearray(binc); dropped = 0
    for m in js["meshes"]:
        for p in m["primitives"]:
            if "indices" not in p: continue
            ...
            weld = weld.reshape(-1); n = int(weld.max()) + 1; t = weld[idx]
            g = coo_matrix((np.ones(2 * len(t)), (np.r_[t[:, 0], t[:, 1]], np.r_[t[:, 1], t[:, 2]])), shape=(n, n))
            root = connected_components(g, directed=False)[1][t[:, 0]]
            ids, cnt = np.unique(root, return_counts=True)
            keep = np.isin(root, ids[cnt >= frac * len(t)])
            dropped += int((~keep).sum())
            new = idx[keep].astype(np.uint32).reshape(-1)
            out += b"\0" * (-len(out) % 4)
            js["bufferViews"].append({"buffer": 0, "byteOffset": len(out), "byteLength": new.nbytes, "target": 34963}); out += new.tobytes()
            js["accessors"].append({"bufferView": len(js["bufferViews"]) - 1, "componentType": 5125, "count": int(new.size), "type": "SCALAR"})
            p["indices"] = len(js["accessors"]) - 1
    js["buffers"] = [{"byteLength": len(out)}]
    print(f"  dropped {dropped} triangles in small pieces", file=sys.stderr)
    return js, bytes(out)
...
def build(ticker, job, exe, yaw):
    ...
    def fetch(name, url, jid):
        raw = CACHE / name; stamp = raw.with_suffix(".job")
        if not raw.exists() or not stamp.exists() or stamp.read_text() != jid:
            ...
            stamp.write_text(jid)
        return raw
    raw = fetch(f"{ticker}.raw.glb", job["url"], job.get("model_job", ""))
    # "lo_url"/"lo_model_job": the far copy is made from a separate (e.g. Hunyuan LowPoly) model,
    # used when simplifying the full model smears its texture.
    raw_lo = fetch(f"{ticker}.lo.raw.glb", job["lo_url"], job.get("lo_model_job", "")) if job.get("lo_url") else raw
    info = {}
    # Per-job overrides for dense sources (Hunyuan3D v3 gives ~500k faces): "si"/"si_lo" simplify
    # ratios, "tex"/"tex_lo" texture sizes, "drop_small" drops disconnected pieces under that
    # fraction of the triangles, "sv_lo" keeps UVs in mind (-sv) so a far copy's texture
    # does not smear, "sa_lo" aggressive simplification (-sa) for a far copy
    ...
    si_lo = ["-si", str(job.get("si_lo", 0.25))] + (["-sa"] if job.get("sa_lo") else []) + (["-sv"] if job.get("sv_lo") else [])
    ...
    for tag, tex, extra, budget in (("", job.get("tex", 1024), si_hi, hi_budget), ("-lo", job.get("tex_lo", 512), si_lo, BUDGET_HD_LO if job.get("hd") else BUDGET_LO)):
        js, binc = read_glb((raw_lo if tag else raw).read_bytes())
        if job.get("drop_small"): js, binc = drop_small(js, binc, job["drop_small"])
        ...
        if not tag: info.update(dims)  # index.json dims come from the full model
    return info
