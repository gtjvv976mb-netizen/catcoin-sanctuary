...
Retry (2026-09-26): the 8 failed cats all succeeded on the first retry with the same four view jobs and `face_count: 300000`
(19 credits each). Higgsfield reported only `status: failed` with no error text for the earlier jobs; their params matched
jobs that succeeded, so the failures look transient on the provider side. The earlier failed ids are kept as `failed_model_jobs`.
Redo: PATCHPAW's standing reference was tabby-striped, so it was remade with a calico prompt ("mostly white, solid orange
and black patches, no tabby stripes"), along with new views (the back view needed a stronger "seen from behind" prompt).
COUCHCAP uses `drop_small: 0.02`, which drops disconnected pieces under 2% of the triangles (a 3k-triangle fragment above
the head). TRIMCAT's far copy smeared when simplified, so it is made from a separate Hunyuan `generate_type: LowPoly` model
(`lo_url`, `lo_model_job`, 18 credits). `sv_lo` adds gltfpack -sv. index.json dims now come from the full model only.
