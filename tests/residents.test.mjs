...
test("no token yet: every planned cat, not launched, with no mint, no buy link and no explorer link", async () => {
  ...
  assert.deepEqual(asked.map((a) => a.url).sort(), [`${BASE}data/adoptables.json`, `${BASE}data/collection.json`, `${BASE}data/famous.json`, `${BASE}data/planned.json`, `${BASE}data/wallets.json`]);
